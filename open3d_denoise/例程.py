...
def RRO_func(cloud_point):
    ...
    :return:cl:降噪后剩余点云数量；ind：保留数据索引
    """
    cl, ind = cloud_point.remove_radius_outlier(nb_points=16, radius=0.05)
    inlier_cloud = cloud_point.select_by_index(ind, invert=True)  # 索引筛选点云数据中的点
    outlier_cloud = cloud_point.select_by_index(ind)
    # 上色区分
    # outlier_cloud.paint_uniform_color([1, 0, 0])
    # inlier_cloud.paint_uniform_color([0.8, 0.8, 0.8])
    return inlier_cloud, outlier_cloud
