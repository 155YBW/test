...
def server():
    ...
    while True:
        ...
        pcd = o3d.io.read_point_cloud('get.ply')
...
def create_point_pic_head():
    # 创建点云图文件
    with open('get.ply', "w+", encoding='gbk') as f:
        f.write('ply\n')
        f.write('format ascii 1.0\n')
        f.write('comment made by Greg Turk\n')
        f.write('comment this file is a cube\n')
...
def update_point_file(point_num, recv_data):
    # 点云数据更新
    with open('get.ply', 'r+', encoding='gbk') as f:
        data = f.readlines()
        # print(len(data))
        point_num = point_num + 1
        # print(f'p : {point_num}')
        data[4] = f'element vertex {point_num}\n'
    with open('get.ply', "w+", encoding='gbk') as f:
        for i in data:
            f.write(i)
...
def update_point_pic(pointcloud, vis, to_reset):
    # 刷新点云显示
    pcd = o3d.io.read_point_cloud("get.ply")  # 此处读取的pcd文件,也可读取其他格式的
